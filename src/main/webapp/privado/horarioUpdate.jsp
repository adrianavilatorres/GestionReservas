...
<%@ include file="../header.jsp" %>
    ...
    <div align="center">
        <label for="start">Fecha reserva: </label>
        <input onchange="mostrarHorarios()" type="date" id="datePickerId"/> 
    </div>
    <script>
        var dia = new Date();
        var semana = new Date();
        semana.setDate(dia.getDate()+14);
        datePickerId.max = semana.toISOString().split("T")[0];
...
function mostrarHorarios(){
    ...
    let url = 'http://localhost:9090/rest/api/horario/instalacionSinReservar/'+instalacionID+','+$("#datePickerId").val();
    // 
    $.ajax({
            type : 'GET',
            url : url,
...
function formularioR(id){
    datos = "horario="+id+"&fecha="+$("#datePickerId").val();
    var url = 'http://localhost:9090/rest/api/reservass/'+id+','+$("#datePickerId").val();
